...
public class MySQLDatabase implements Database {
    ...
    public void insertMessageToDB(List<ConsumerRecord<String, MessageObject>> records) throws SQLException {
        ...
        String query = "INSERT IGNORE INTO `products`(`pogid`, `supc`, `price`, `quantity`) VALUE (?, ?, ?, ?)";
	    ...
	    String kafkaRecord = record.value().getMessage();
	    String splitKafkaRecord = kafkaRecord.split("|");
            preparedStatementInsert.setString(1, splitKafkaRecord[0]);
            preparedStatementInsert.setString(2, splitKafkaRecord[1]);
            preparedStatementInsert.setString(3, splitKafkaRecord[2]);
            preparedStatementInsert.setString(4, splitKafkaRecord[3]);
